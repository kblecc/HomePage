...
function build_bm_area(datafp) {
  ...
  for (let i = 0; i < Object.keys(data).length; i++) {
    ...
    htmlparse =
      htmlparse +
      '<div class="ms-2 c-details"><h3 class="mb-0">' +
      data[i].title +
    ...
    for (let j = 0; j < Object.keys(data[i]).length; j++) {
      htmlparse =
        htmlparse +
        '<a ><span class="badge bg-secondary" onclick="filter_by_tag($(this).text());event.stopPropagation();">' +
        data[i].tags[j] +
        '</span></a>';
    }
    htmlparse = htmlparse + '</div></div></div>';
  }
  htmlparse = htmlparse + show_container('', htmlparse);
}
...
function build_tag_page() {
  ...
  for (let i = 0; i < Object.keys(data).length; i++) {
    htmlparse =
      htmlparse +
      '<button type="button" class="btn btn-primary" onclick="filter_by_tag($(this).text());">' +
      data[i] +
      '</button>';
  }
